        ...
        <ul class="inline-flex items-center -space-x-px">
            ...
            @if ($paginator->onFirstPage())
                <li class="disabled" aria-disabled="true">
                    <span class="inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-not-allowed leading-5 rounded-l-md">
                        &lsaquo;
                    </span>
                </li>
            @else
            ...
            @foreach ($elements as $element)
                ...
                @if (is_string($element))
                    <li class="disabled" aria-disabled="true">
                        <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 cursor-not-allowed leading-5">
                            {{ $element }}
                        </span>
                    </li>
                @endif
                    ...
                    @foreach ($element as $page => $url)
                        @if ($page == $paginator->currentPage())
                            <li class="active" aria-current="page">
                                <span class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-500 border border-blue-500 leading-5">
                                    {{ $page }}
                                </span>
                            </li>
                        @else
            ...
            @else
                <li class="disabled" aria-disabled="true">
                    <span class="inline-flex items-center px-2 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 cursor-not-allowed leading-5 rounded-r-md">
                        &rsaquo;
                    </span>
                </li>
            @endif
